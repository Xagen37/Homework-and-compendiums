...
struct Trie
{
  ...
  long long int count_substr()
  {
    vector <bool> used(nodes.size(), 0);
    vector <long long int> dp(nodes.size(), 1);
    dfs(0, used, dp);
    return dp[0] - 1;
  }

  void dfs(int v, vector <bool> & used, vector <long long int> & dp)
  {
    used[v] = true;
    dp[v] = 1;
    for (auto u : nodes[v].next)
    {
      if (!used[u.second])
      {
        dfs(u.second, used, dp);
      }
      dp[v] += dp[u.second];
    }
  }
